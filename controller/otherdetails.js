...
exports.adddetails = async (req, res) => {    
    ...
    const FIND_USER = await OtherDetails.findOne({ MatriID: MatriID })
    if (FIND_USER) {
...
exports.setting = async (req, res) => {
    ...
    const updatedChange = await OtherDetails.findOneAndUpdate({ _id: req.params.id }, { $set: req.body },{new: true})
    .then((updateddetails)=>{
        res.json({
...
exports.viewdetails = async (req,res)=>{
    const finddetails = await OtherDetails.findOne({ _id: req.params.id })
    if (finddetails) {
